...
describe('PLACEHOLDER_SCENARIO_DEFS bindings', () => {
  ...
  it('only binds create templates that actually render a carousel', () => {
    ...
    const SUPPORTED = new Set(['deck', 'image']);
...
describe('buildPlaceholderScenarios', () => {
  function idsFor(activeChipId: string | null) {
  ...
  it('shows the selected deck lines without mixing in image lines', () => {
    expect(idsFor('deck')).toEqual(['notes-to-deck', 'team-update-slides', 'pitch-deck']);
    expect(idsFor('image')).toEqual(['product-detail', 'landing-intro', 'landing-layout']);
  });

  it('keeps each rotating line bound to its current top-level template', () => {
    const rotation = buildPlaceholderScenarios({
      activeChipId: null,
      resolveTextKey: (key) => en[key],
    });
    expect(rotation.find((scenario) => scenario.id === 'notes-to-deck')).toMatchObject({
      chipId: 'deck',
    });
    expect(rotation.find((scenario) => scenario.id === 'product-detail')).toMatchObject({
      chipId: 'image',
    });
  });
  ...
  it('uses prompt examples as selected-chip carousel scenarios when no curated scenario exists', () => {
    ...
    expect(scenarios).toEqual([
      {
        id: 'custom-prompt-example-1',
        chipId: 'custom',
        text: 'Create a product visual',
      },
    ]);
  });
  ...
  it('creates a submittable selected-chip fallback when neither curated scenarios nor prompt examples exist', () => {
    const scenarios = buildPlaceholderScenarios({
      ...
      fallbackForChip: (chipId) => (
        chipId === 'custom'
          ? 'Create an image: Product campaign visual'
          : null
      ),
    });
    ...
    expect(scenarios).toEqual([
      {
        id: 'custom-fallback',
        chipId: 'custom',
        text: 'Create an image: Product campaign visual',
      },
    ]);
  });
});
