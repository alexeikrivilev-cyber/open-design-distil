...
/**
 * The page composer is kept mounted while other entry views are visible, so a
 * one-shot latch plus a DOM event lets those views request a chip without
 * coupling them to Home's React state.
 */
let pendingChipId: string | null = null;

// Queue a composer chip to auto-select on the next render, then notify any
// mounted HomeView. Safe to call before HomeView
...
export function requestHomeChip(chipId: string): void {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent(HOME_CHIP_INTENT_EVENT, { detail: { chipId } }),
    );
  }
}
